...
class ValueInquiry(Node):
    ...
    def assess_instrumental_effectiveness(self) -> Dict[str, float]:
        ...
        if self.evidence_gathering and self.analysis_methods:
            knowledge_score = min(len(self.evidence_gathering) * len(self.analysis_methods) / 10.0, 1.0)
...
class NormativeEvaluation(Node):
    ...
    def _identify_tension_type(self, criteria1: NormativeCriteria, criteria2: NormativeCriteria) -> Optional[str]:
        """Identify type of tension between two criteria."""
        tension_patterns = {
            (NormativeCriteria.ECONOMIC_EFFICIENCY, NormativeCriteria.SOCIAL_EQUITY): "efficiency_equity_tension",
            (NormativeCriteria.ECONOMIC_EFFICIENCY, NormativeCriteria.ECOLOGICAL_SUSTAINABILITY): "economy_environment_tension",
            (NormativeCriteria.CULTURAL_CONTINUITY, NormativeCriteria.ADAPTIVE_CAPACITY): "tradition_change_tension",
            (NormativeCriteria.DEMOCRATIC_PARTICIPATION, NormativeCriteria.ECONOMIC_EFFICIENCY): "democracy_efficiency_tension"
        }
    ...
    def _suggest_tension_resolution(self, criteria1: NormativeCriteria, criteria2: NormativeCriteria) -> List[str]:
