...
class CriteriaApplication(Node):
    ...
    def calculate_aggregated_scores(self, aggregation_method: AggregationMethod = AggregationMethod.WEIGHTED_AVERAGE) -> Dict[uuid.UUID, float]:
        ...
        for criterion_id in self.applied_criteria:
            if criterion_id in self.criteria_weights:
                ...
                for new_weight in weight_variations:
                    ...
                    ranking_correlation = self._calculate_ranking_correlation(original_rankings, temp_rankings)
    ...
    def _calculate_ranking_correlation(self, ranking1: List[uuid.UUID], ranking2: List[uuid.UUID]) -> float:
    ...
    def validate_application_results(self) -> Dict[str, Any]:
        ...
        if self.aggregated_scores and self.entity_rankings:
            ...
            score_based_ranking = sorted(self.aggregated_scores.items(), key=lambda x: x[1], reverse=True)
...
class MultiCriteriaAnalysis(Node):
    ...
    def _calculate_consensus_ranking(self, method_rankings: Dict[str, List[uuid.UUID]]) -> List[uuid.UUID]:
    ...
    def _identify_robust_alternatives(self, method_rankings: Dict[str, List[uuid.UUID]]) -> List[uuid.UUID]:
