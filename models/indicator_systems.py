...
class IndicatorRelationship(Node):
    ...
    def validate_relationship(self) -> Dict[str, Any]:
        ...
        validation_results['theoretical_validity'] = min(validation_results['theoretical_validity'], 1.0)
...
class PerformanceMeasurement(Node):
    ...
    def calculate_composite_performance(self) -> Dict[str, float]:
        ...
        for category, indicator_ids in self.performance_categories.items():
            category_scores = [self.current_performance.get(ind_id, 0.0) for ind_id in indicator_ids]
            if category_scores:
    ...
    def generate_performance_report(self) -> Dict[str, Any]:
        ...
        performance_report['executive_summary'] = {
            ...
            'performance_level': self._categorize_performance(composite_scores.get('overall_performance', 0.0))
        }
...
class IndicatorSystem(Node):
    ...
    def assess_system_completeness(self) -> Dict[str, Any]:
        ...
        outcome_indicators = len(self.indicator_categories.get(IndicatorCategory.OUTCOME_INDICATOR, []))
        process_indicators = len(self.indicator_categories.get(IndicatorCategory.PROCESS_INDICATOR, []))
        
        completeness_assessment['outcome_coverage'] = min(outcome_indicators / 10.0, 1.0)  # Assume 10 outcomes needed
        completeness_assessment['process_coverage'] = min(process_indicators / 15.0, 1.0)  # Assume 15 processes
